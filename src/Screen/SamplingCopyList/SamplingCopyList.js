...
export default function SamplingCopyList({route, navigation}) {
  ...
  const getRigionsDepotList = async (region, responseData) => {
    ...
    const token = await AsyncStorage.getItem('InExToken');
    //const userId = await AsyncStorage.getItem('InExUserId');
    const response = await auth.regionWiseParcelAndDepotForSampling(
      sendingData,
      token,
    );
    ...
    return (
      <View style={styles.listItemContainer}>
        {item?.date_type == 'weekend' ? (
          ...
          <>
            ...
            <View
              ...
              <Text style={{color: COLORS.black, fontSize: 16}}>
                To Date: {item?.to_show_formatted_to_date} {converteTodDate}
              </Text>
            </View>
          </>
        )}
        ...
        {item?.editable ? (
          ...
          item?.status == 'PENDING' ? null : userDetail?.role ===
            'Regional Manager' &&
